...
volatile int sx = 128 / 2 - 4, sy = 64 / 2 - 4, svx = 0, svy = 0;
 
volatile int bx = 128 / 2 - 8, by = 64 - 8;
volatile int bvx = 0;
volatile int16_t uval = 0;
 
int ux = 2, uy = 2;
volatile _Bool started = 0, gameover = 0;
...
void ADC_IRQHandler(void)
{
    ...
    if (u32Flag & ((1ul << 0)))
    {
        u16_val = ((((ADC_T *) ((( uint32_t)0x40000000) + 0xE0000)))->ADDR[(0)] & (0xFFFFul << 0)) + 2048;
        uval = u16_val;
    }
...
void Show()
{
    ...
    if (started)
        for (size_t x_offset = 0; x_offset < 8; x_offset++)
            draw_Line(sx + x_offset, sy, sx + x_offset, sy + 8, 0xFFFF, 0x0000);
     
    for (size_t x_offset = 0; x_offset < 16; x_offset++)
        draw_Line(bx + x_offset, by, bx + x_offset, by + 8, 0xFFFF, 0x0000);
     
    for (size_t x_offset = 0; x_offset < 16; x_offset++)
        draw_Line(ux + x_offset, uy, ux + x_offset, uy + 8, 0xFFFF, 0x0000);
    if (gameover)
        printS(128 / 2 - 32, 64 / 2 - 4, "GAMEOVER");
    CLK_SysTickDelay(100000);
     
    if (started)
        for (size_t x_offset = 0; x_offset < 8; x_offset++)
            draw_Line(sx + x_offset, sy, sx + x_offset, sy + 8, 0x0000, 0x0000);
     
    for (size_t x_offset = 0; x_offset < 16; x_offset++)
        draw_Line(bx + x_offset, by, bx + x_offset, by + 8, 0x0000, 0x0000);
     
    for (size_t x_offset = 0; x_offset < 16; x_offset++)
        draw_Line(ux + x_offset, uy, ux + x_offset, uy + 8, 0x0000, 0x0000);
}
